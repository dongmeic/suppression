...
ncin <- nc_open(paste(ncpath,"wus10km_v1.nc",sep=""))
...
temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,40:56])  

# create netCDF file and put data
ncfname <- paste(ncpath,"wus10km_v1_short_01.nc", sep="")
dunits <- "binary"
var_def <- ncvar_def("btlfir_prs", dunits, list(xdim, ydim, tdim), fillvalue,"beetle and fire coorrurence in western US", prec="float")
...
for (i in 1:length(varlist)){
  ...
  temp_array <- array(fillvalue, dim = c(nx,ny,nt))
  temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,(7+nt*(i-1)):(6+nt*i)])  
  ncvar_put(ncout, var_def, temp_array)
}
